...
function computeCompleteness(sections) {
  ...
  for (const key of REQUIRED_SECTIONS) {
    ...
    // On attend ~3 réponses minimum par section côté heuristique v1
    const expected = 3;
...
function computeDetail(sections, sectionNotes) {
  ...
  // 5 sections, ~80 chars idéal par section ⇒ 400 chars
...
function computeConsistency(sections) {
  // Détection de réponses uniformes/baclées : trop de fois la même valeur
  const allValues = [];
  for (const key of REQUIRED_SECTIONS) {
    const section = safeObj(sections[key]);
  ...
  // Suspect : très peu de diversité ET peu de réponses au total (signal "baclé")
